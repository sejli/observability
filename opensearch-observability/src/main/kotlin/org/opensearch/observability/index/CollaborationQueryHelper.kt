 ...
 */
internal class CollaborationQueryHelper(private val types: EnumSet<CollaborationObjectType>) {
    companion object {
        ...
        // keyword and text fields are under collaboration object and should be prepended with type
        ...
        private val COLLABORATION_OBJECT_FIELDS = KEYWORD_FIELDS.union(TEXT_FIELDS)
        private val ALL_FIELDS = METADATA_FIELDS.union(COLLABORATION_OBJECT_FIELDS)

        val FILTER_PARAMS = ALL_FIELDS.union(setOf(QUERY_FIELD))
    }

    private val prefixes = if (types.size == 0) {
        CollaborationObjectType.getAll()
    } else {
        types
    }
    ...
    private fun getQueryAllBuilder(queryValue: String): QueryBuilder {
        ...
        COLLABORATION_OBJECT_FIELDS.forEach {
